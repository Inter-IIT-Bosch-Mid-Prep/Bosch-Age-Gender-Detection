...
def restore(image, model, device):
    ...
    input_ = TF.to_tensor(image).unsqueeze(0)
    input_=(input_).to(device)
    #print("converted",input_)
    #cv2.imwrite( "/content/sample_data/image2.png", (input_).permute(0, 2, 3, 1).cpu().detach().numpy().squeeze())
